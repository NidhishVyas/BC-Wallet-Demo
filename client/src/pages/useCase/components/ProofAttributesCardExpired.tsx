...
export const ProofAttributesCardExpired: React.FC<Props> = ({ entityName, requestedCredentials, proof, proofReceived }) => {
          ...
          {item.properties?.map((prop: string) => {
            ...
            return (
              <div key={prop} className="flex flex-row" >
                ...
                >
                  <p>
                    {(prop.charAt(0).toUpperCase() + prop.slice(1)).replace('_', ' ') != 'Vehicle information'
    ...
    <div className="flex flex-col bg-bcgov-white dark:bg-bcgov-black p-4 md:mb-8 rounded-lg shadow max-h-64 my-2 sm:max-h-72 md:max-h-96 overflow-auto">
      <div className="flex-1-1 title">
        <div className="flex flex-row">
          ...
          <div className="flex-1-1 h-8 mb-2">{proofReceived ? <CheckMark /> : <CrossMark />}</div>
        </div>
